        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="min-w-0">
            <div className="flex items-center mb-2">
              <Bike className="h-5 w-5 mr-2 text-primary flex-shrink-0" />
              <h3 className="text-lg font-medium text-primary truncate">{ride.name}</h3>
            </div>
            <div className="text-sm text-secondary">
              {formatDate(ride.start_time)}
            </div>
          </div>
          {/* Route shape, read straight off the row (migration 011). Hidden
              on narrow screens where the checkbox overlay would collide. */}
          <RouteThumbnail
            routePath={ride.route_path}
            className="hidden sm:block flex-shrink-0 opacity-70 group-hover:opacity-100 transition-opacity mr-10"
          />
        </div>
