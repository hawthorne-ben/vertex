 ...
 *
 * Single-pass computation of all IMU-derived metrics:
 * stability, roughness, riding position, braking.
 * All processing done in single step to avoid Inngest output size limits (512KB).
 */
export const analyzeRideImuJob = inngest.createFunction(
  {
    id: 'analyze-ride-imu',
    retries: 3,
    concurrency: {
    ...
    try {
      ...
      const existingAnalyses = await step.run('check-existing-analyses', async () => {
        const { data } = await supabase
          ...
          .select('*')
          .eq('ride_id', rideId)
          .in('analysis_type', ['pedaling_efficiency', 'riding_position', 'surface_roughness', 'braking'])
      ...
      const brakingAnalysis = existingAnalyses.find(a => a.analysis_type === 'braking')
      ...
      const allUpToDate =
        ...
        brakingAnalysis?.status === 'completed' &&
        brakingAnalysis?.algorithm_version === C.ALGORITHM_VERSION
      ...
      if (allUpToDate) {
        return {
          ...
          brakingAnalysisId: brakingAnalysis.id,
        }
      }
      ...
      const result = await step.run('compute-and-store-analyses', async () => {
        ...
        const brakingAnalysisId = brakingAnalysis.id
        ...
        return {
          ...
          brakingSampleCount: computeResult.braking.samples.length,
          ...
          totalBrakingEvents: computeResult.braking.metadata.totalBrakingEvents,
          brakingPercent: computeResult.braking.metadata.brakingPercent,
          avgBrakingIntensity: computeResult.braking.metadata.avgBrakingIntensity,
          maxBrakingIntensity: computeResult.braking.metadata.maxBrakingIntensity,
        }
      })
      ...
      await step.run('upsert-ride-summary', async () => {
        ...
        const summaryRow = {
          ...
          // IMU-derived: Braking
          total_braking_events: result.totalBrakingEvents ?? null,
          braking_percent: result.brakingPercent ?? null,
          avg_braking_intensity: result.avgBrakingIntensity ?? null,
          max_braking_intensity: result.maxBrakingIntensity ?? null,

          // Algorithm versions
          efficiency_version: C.ALGORITHM_VERSION,
          position_version: C.ALGORITHM_VERSION,
          roughness_version: C.ALGORITHM_VERSION,
          braking_version: C.ALGORITHM_VERSION,
